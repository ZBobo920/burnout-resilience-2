...
function getRatingForStatement(map, statement, sortedStatements) {
    ...
    if (!sortedStatements || !sortedStatements.grid) {
        return '';
    }
    if (!map || !map.column) {
        return '';
    ...
    for (var i = 0; i < sortedStatements.grid.length; i++) {
        if (!sortedStatements.grid[i]) continue;
        for (var j = 0; j < sortedStatements.grid[i].length; j++) {
            var gridItem = sortedStatements.grid[i][j];
            if (!gridItem || !gridItem.statement) continue;
            
            if (gridItem.statement._id === statement._id) {
                if (map.column[i]) {
